...
def _event_to_dict(session: Session, event: Event) -> dict:
    labels = session.scalars(
        select(EventLabel.label).where(EventLabel.event_id == event.id)
    ).all()
...
class EventStore:
        ...
        *,
        ...
        with self.session_factory.begin() as session:
            ...
            for label in dict.fromkeys(labels or []):
                session.add(EventLabel(event_id=event.id, label=label))
            session.flush()
        ...
        *,
        ...
        with self.session_factory() as session:
            ...
            if label is not None:
                query = query.join(EventLabel, EventLabel.event_id == Event.id).where(
        ...
        with self.session_factory.begin() as session:
            ...
            if type is not None and type != event.type:
                ...
                if has_dependent_data is not None:
                    raise EventTypeLockedError(
                        "Cannot change event type once assignments exist"
                    )
                event.type = type
    ...
    def delete_event(self, event_id: int) -> bool:
        ...
        Returns True if the event was deleted (either way), False if it
        ...
        with self.session_factory.begin() as session:
            ...
            existing = session.scalar(
                select(Assignment).where(
                    Assignment.event_id == event_id, Assignment.user_id == user_id
                )
            )
